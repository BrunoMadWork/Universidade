...
ArrayList<Long> tempos;
	public void run()
    {
		...
		*************************************************************************************/
		...
		String	fileout = "OutputB.dat";  // Output file
		String  filerejeitados = "WildPoints.dat";
		String  tempOutput = "";	
		String  altOutput = "";		
		String psiOutput = "";
		DataOutputStream out = null;
		DataOutputStream outrej = null;		
		byte databyteout = 0;
		...
		NumberFormat tempFormat = new DecimalFormat("#000.00000"); 
		NumberFormat altFormat = new DecimalFormat("#000000.00000"); 
		NumberFormat psiFormat = new DecimalFormat("#000.00000");

		try
		{
			out = new DataOutputStream(new FileOutputStream(fileout));
			outrej = new DataOutputStream(new FileOutputStream(filerejeitados));
			...
			try
			{
				...
				****************************************************************************/
				...
				if ( id == 0 )
				{
					...
					paracomparar = measurement;
					if (tempos.isEmpty())
					{
						tempos.add(measurement);
					}
					ultimo = tempos.get(tempos.size() - 1);
					if(measurement > ultimo)
					{
				...
				****************************************************************************/
				...
				if ( id == 4 )
				{
					...
					tempOutput = formato.format(TimeStamp.getTime()) + "   " + tempFormat.format(Double.longBitsToDouble(measurement));

					try{
						out.writeBytes(tempOutput);
					}
					catch(IOException e)
					{
						// IOException
                    ...
                    {
                        try{
                            out.writeBytes(altOutput);
							out.writeBytes(psiOutput);
                        }
                        catch(IOException e)
                        {
                            // IOException
				...
				if( id == 2){
					System.out.print( TimeStampFormat.format(TimeStamp.getTime()) + " ID = " + id + "  " + Double.longBitsToDouble(measurement) +"\n");
					altOutput = "   " + altFormat.format(Double.longBitsToDouble(measurement));
				...
				if( id == 3)
				{
					if(tempos.contains(paracomparar))
					{
						System.out.print( TimeStampFormat.format(TimeStamp.getTime()) + " ID = " + id + "  " + Double.longBitsToDouble(measurement) + "*" + "\n");
						
						psiOutput = "   " + psiFormat.format(Double.longBitsToDouble(measurement))+ "*" + "\n";
				...
				if(id == 7)
				{
					...
					psiOutput = TimeStampFormat.format(TimeStamp.getTime()) + "   " + psiFormat.format(Double.longBitsToDouble(measurement)) + "\n";

						
							try{
								outrej.writeBytes(psiOutput);
							}
							catch(IOException e)
							{
								// IOException
